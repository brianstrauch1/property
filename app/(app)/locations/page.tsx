...
export default function LocationsPage() {
  ...
  const [addParentId, setAddParentId] = useState<string | null>(null) // null => root
  ...
  /* ------------------------------ delete (C: block unless subtree empty) ------------------------------ */

  const canDeleteSubtree = (id: string) => {
    const subtree = descendantsOf(id)
    return subtree.every(locId => (countMap[locId] || 0) === 0)
  }

  const deleteLocation = async (id: string) => {
    if (!property?.id) return
    const subtreeEmpty = canDeleteSubtree(id)
    if (!subtreeEmpty) {
      return alert('Cannot delete: this location (or one of its sub-locations) contains inventory items.')
    }

    // Also block if it has children (even empty) unless you want cascading delete.
    // Per your C-rule, we still should avoid surprises; we require subtree empty, so children are empty too.
    // We'll allow delete only if it has no children, to avoid orphaned rows.
    const hasChildren = (childrenMap[id] || []).length > 0
    if (hasChildren) {
      return alert('Cannot delete: this location has child locations. Delete/move children first.')
    }
  ...
  /* ------------------------------ drag & drop reorder + reparent with sort_order ------------------------------ */
  ...
  function TreeNode({ node, depth }: { node: LocationRow; depth: number }) {
    ...
    const subtreeEmpty = canDeleteSubtree(node.id)
    const canDeleteLineItem = subtreeEmpty && !hasChildren
      ...
      <div ref={setNodeRef} style={style}>
        ...
        >
          ...
          {!isInline ? (
            ...
            >
              {node.name} <span className="text-slate-500 font-normal">({subtreeCount})</span>
            </span>
          ) : (
            <input
              autoFocus
          ...
          <div className="ml-auto flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button
              className={[
                ...
                canDeleteLineItem ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-slate-100 text-slate-400 cursor-not-allowed'
              ].join(' ')}
              onClick={(e) => {
                e.preventDefault()
                e.stopPropagation()
                if (!canDeleteLineItem) return
            ...
            <div
              {...listeners}
              {...attributes}
              className="cursor-grab text-slate-400 text-xs px-2"
    ...
    <main>
      <div className="bg-white p-6 rounded-xl shadow-md">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            ...
            <div className="text-sm text-slate-500 mt-1">
              Counts show total items in the subtree. Delete is allowed only when the subtree is empty.
            </div>
          </div>
        ...
        <DndContext
          ...
          onDragEnd={async (event) => {
            ...
            await reorderOrReparent(String(active.id), String(over.id))
          }}
        >
          {roots.length === 0 ? (
      ...
      {ctxMenu && (
        <div
          ...
          style={{ left: ctxMenu.x, top: ctxMenu.y, width: 240 }}
          onClick={(e) => e.stopPropagation()}
        >
          <button
            className="w-full text-left px-3 py-2 hover:bg-slate-50"
            onClick={() => openAddChild(ctxMenu.locationId)}
          >
            + Add Child Location
          </button>
          <button
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-xl shadow-lg w-[520px] p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-bold text-slate-900 mb-1">
              {addParentId ? 'Add Child Location' : 'Add Root Location'}
            </h2>
            <div className="text-sm text-slate-500 mb-4">
              {addParentId ? `Parent: ${byId[addParentId]?.name ?? ''}` : 'Creates a top-level location.'}
            </div>
            ...
            <input
              ...
              placeholder="e.g., First Floor / Garage / Office"
              autoFocus
            />
